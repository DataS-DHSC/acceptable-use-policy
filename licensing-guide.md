...
However, for open source code to be useful, users have to know how they can re-use and adapt it. The licence specifies this. 
On Github, you can add a licence when creating a new repository, or later on by adding a LICENCE.md file. 

## Deciding your licence

For DHSC work, the default licence is the [OGLv3.0, as published by the national archives](https://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/). 
However, there may be reasons other licences are more appropriate.
These are the steps to work out what licence your code should have.

1. Make a list of all package dependencies of your code, and the licences those packages were published under. The package's PyPi (Python) or CRAN (R) page should list this. For example, [dplyr is published under the MIT licence](https://cran.r-project.org/web/packages/dplyr/index.html).
2. For each package licence, check whether the licence requires publishing of new or modified code under the same licence. You should carefully read the conditions of the licences for the packages which are being depended on before publishing your code.  Please note that the licences may place more obligations on DHSC than a simple requirement to use the same licence and it’s important to check for these.       
3. Would there be a technical benefit to publishing your code under a licence other than the OGLv3? We suggest looking at latest tests set out by [The National Archives](https://www.nationalarchives.gov.uk/information-management/re-using-public-sector-information/uk-government-licensing-framework/open-government-licence/open-software-licences/). For example, using a different licence may help promote adoption by the software community or promote adoption to benefit UK industry and consumers. 
4. If there would be a technical benefit from publishing your code under a licence other than OGLv3, you may choose an [alternative OSI approved licence](https://opensource.org/licences). You probably want to consider the MIT licence. 
